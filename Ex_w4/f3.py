...
# ==== 基因演算法會用到的函式 ====
def initPop():             # 初始化群體
    # 產生 NUM_CHROME * NUM_BIT 個[-32, 32]之間的隨機數
	return np.random.uniform(low = -32, high = 32, size = (NUM_CHROME,NUM_BIT)) 


def fitFunc(x):            # 適應度函數
    a = 20 * math.exp( (-0.2) * math.sqrt( (1/NUM_BIT) * sum(a*a for a in x))) 
    b = math.exp( (1/NUM_BIT) * sum( math.cos( 2 * math.pi * a) for a in x))
    return a + b -20 - math.exp(1)
